"""Plan comptable SYSCOHADA révisé — référentiel de comptes chargé par société.

Liste des comptes principaux du Système comptable OHADA révisé (classes 1 à 8).
Ce n'est pas la totalité exhaustive des comptes divisionnaires possibles, mais le
socle professionnel réellement utilisé, que le comptable étend ensuite librement
via l'écran « Plan comptable ».

La classe est déduite du 1er chiffre du numéro. Les comptes collectifs de tiers
(401 fournisseurs, 411 clients, …) sont marqués `auxiliaire` : ils se ventilent
par tiers dans le grand livre auxiliaire.
"""
...
def charger_plan(db: Session, societe_id, remplacer: bool = False) -> int:
    """Insère les comptes SYSCOHADA manquants pour une société.

    N'écrase jamais un compte déjà présent (préserve intitulés personnalisés).
    Retourne le nombre de comptes créés.
    """
    from . import models

    existants = set(db.execute(
        select(models.Compte.numero).where(models.Compte.societe_id == societe_id)
    ).scalars().all())
    crees = 0
    for numero, intitule in PLAN_SYSCOHADA:
        if numero in existants:
            continue
        db.add(models.Compte(
