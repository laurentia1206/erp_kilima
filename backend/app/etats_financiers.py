"""États financiers SYSCOHADA (Système Normal) — calculés à partir du grand livre.

- Compte de résultat : cascade des soldes caractéristiques (exploitation →
  financier → activités ordinaires → HAO → résultat net), colonnes N / N-1.
- Bilan : Actif (immobilisé, circulant, trésorerie) = Passif (capitaux propres
  incluant le résultat de l'exercice, dettes financières, passif circulant,
  trésorerie passif), colonnes N / N-1.
- TFT : Tableau des flux de trésorerie (opérationnel, investissement,
  financement) reliant la trésorerie d'ouverture à celle de clôture.

Tout part du solde (débit − crédit) de chaque compte. Les produits (classe 7)
sont exprimés en positif (crédit − débit), les charges (classe 6) en positif
(débit − crédit).
"""
...
def _rows(db: Session, societe_id: uuid.UUID, statut: str | None = None) -> list[tuple]:
    ...
    return [(c, s, float(m), d) for c, s, m, d in db.execute(q).all()]
...
def _soldes(db: Session, societe_id: uuid.UUID, statut: str | None = None) -> dict[str, float]:
    """Solde (D − C) par compte, toutes périodes confondues (état courant)."""
    return _agg(_rows(db, societe_id, statut), lambda _d: True)


def _labels(db: Session, societe_id: uuid.UUID) -> dict[str, str]:
    rows = db.execute(select(models.Compte.numero, models.Compte.intitule)
...
def compte_resultat(db: Session, societe_id: uuid.UUID, statut: str | None = None,
                    annee: int | None = None) -> dict:
    annee = annee or date.today().year
    rows = _rows(db, societe_id, statut)
    sN = _agg(rows, lambda d: d.year == annee)          # produits/charges = flux de l'exercice
    sN1 = _agg(rows, lambda d: d.year == annee - 1)
    labels = _labels(db, societe_id)
...
def bilan(db: Session, societe_id: uuid.UUID, statut: str | None = None,
          annee: int | None = None) -> dict:
    annee = annee or date.today().year
    rows = _rows(db, societe_id, statut)
    N = _bilan_chiffres(_agg(rows, lambda d: d.year <= annee), _labels(db, societe_id))
    N1 = _bilan_chiffres(_agg(rows, lambda d: d.year <= annee - 1), _labels(db, societe_id))
    return {
        "annee": annee,
        "actif": {
...
def tft(db: Session, societe_id: uuid.UUID, statut: str | None = None,
        annee: int | None = None) -> dict:
    """Tableau des flux de trésorerie : variation de la trésorerie de l'exercice
    décomposée en flux opérationnels, d'investissement et de financement.

    Chaque flux = − (variation des comptes de contrepartie sur l'exercice) :
      financement = −Δ capitaux & dettes financières (classe 1)
      investissement = −Δ immobilisations (classe 2)
      opérationnel = résultat + Δ non décaissé + variation du BFR (classes 3, 4, 6-8)
    L'identité de la partie double garantit : ouverture + variation = clôture.
    """
    annee = annee or date.today().year
    rows = _rows(db, societe_id, statut)
...
def cockpit(db: Session, societe_id: uuid.UUID) -> dict:
    """Indicateurs financiers de synthèse pour le DAF."""
    soldes = _soldes(db, societe_id)
    ...
    en_attente = db.execute(
        select(models.Ecriture).where(models.Ecriture.societe_id == societe_id,
                                       models.Ecriture.statut == "en_attente")).scalars().all()
    return {
        ...
        "pieces_en_attente": len(en_attente),
    }
