...
class CarburantViewSet(MetierViewSet):
    ...
    def vehicules(self, request):
        """Sélecteur de porteurs, limité au périmètre et aux droits carburant."""
        sid = _societe_param(request)
        _acces(request, sid)
        rows = [{"id": str(v.id), "type": "camion", "nom": v.immatriculation}
                for v in Camion.objects.filter(societe_id=sid, actif=True)]
        rows += [{"id": str(v.id), "type": "engin", "nom": v.nom}
                 for v in Engin.objects.filter(societe_id=sid, actif=True)]
        return Response(sorted(rows, key=lambda v: (v["type"], v["nom"])))
