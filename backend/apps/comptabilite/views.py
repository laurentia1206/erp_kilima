...
from apps.comptabilite import etats_financiers as etats_financiers
from apps.comptabilite import plan_syscohada as plan_syscohada
...
from apps.tresorerie.models import Caisse, MouvementCaisse, Transfert
from apps.comptabilite.models import Compte, Ecriture, Journal, LigneEcriture, RapprochementBancaire, VentilationAnalytique
from core.models import Parametre, PieceJointe, Tiers
...
def _revision_ecriture(e, lignes):
    contenu = [e.statut, str(e.date_ecriture), e.libelle, [
        [str(l.id), l.compte_numero, l.sens, str(l.montant_usd), str(l.tiers_id), l.libelle_ligne]
        for l in sorted(lignes, key=lambda l: str(l.id))]]
    return hashlib.sha256(json.dumps(contenu, ensure_ascii=False).encode()).hexdigest()


def _analytique_compatible(ligne, compte, montant):
    # L'éclatement réutilise l'identifiant de la première part : ne pas laisser
    # ses ventilations existantes dépasser le nouveau montant ni sortir de 6/7.
    par_axe = {}
    for v in VentilationAnalytique.objects.filter(ligne_ecriture_id=ligne.id):
        par_axe[v.axe_id] = par_axe.get(v.axe_id, Decimal('0')) + v.montant_usd
    return not par_axe or (compte[:1] in ('6', '7') and all(v <= montant + Decimal('0.01') for v in par_axe.values()))
...
def _ecriture_dict(e: Ecriture, intitules: dict | None = None) -> dict:
    ...
    return {
        ...
        "revision": _revision_ecriture(e, lignes),
        "lignes": [{"id": str(l.id), "sens": l.sens, "compte": l.compte_numero,
                    ...
                    "tiers_id": str(l.tiers_id) if l.tiers_id else None,
...
class EcritureViewSet(MetierModelViewSet):
    ...
    def saisie(self, request):
        ...
        for l in lignes_in:
            try:
                ...
                return refus({"detail": "Montant de ligne invalide."}, status=422)
            if l.get("sens") not in ("D", "C") or not l.get("compte") \
                    or not isfinite(montant) or montant <= 0:
        ...
        for l in lignes_in:
            ...
            if l.get("tiers_id"):
                ...
                try:
                    tiers_uuid = UUID(str(l["tiers_id"]))
                except (ValueError, TypeError, AttributeError):
                    return refus({"detail": "Identifiant du tiers invalide."}, status=422)
                tiers = Tiers.objects.filter(id=tiers_uuid).first()
                if not tiers_disponible(tiers, sid):
                    return refus({"detail": "Le tiers de cette ligne est inactif ou indisponible dans cette société."}, status=400)
        ...
        try:
            ...
            return refus({"detail": "Date d’écriture invalide."}, status=422)
    ...
    def valider(self, request, ecriture_id):
        ...
        e = Ecriture.objects.select_for_update().filter(id=ecriture_id).first()
        if not e:
        ...
        if payload.get('revision') is not None and payload['revision'] != _revision_ecriture(e, lignes_ecr):
            return refus({'detail': 'Cette pièce a changé. Rechargez-la avant de valider.'}, status=409)
        ...
        with transaction.atomic():
            for sp in payload.get("splits", []):
                ...
                if not repartition:
                    return refus({'detail': 'Une ligne éclatée doit contenir une répartition.'}, status=422)
                if ligne.id in split_ids:
                    return refus({'detail': 'Ligne à éclater fournie plusieurs fois.'}, status=422)
                try:
                    montants = [Decimal(str(r['montant'])) for r in repartition]
                    if any(not m.is_finite() or m <= 0 or m != m.quantize(Decimal('0.01')) for m in montants):
                        raise ValueError()
                except (ValueError, TypeError, KeyError, InvalidOperation):
                    return refus({'detail': 'Montants d’éclatement positifs requis, au centime près.'}, status=422)
                total = sum(montants, Decimal('0'))
                if total != ligne.montant_usd:
                ...
                for r in repartition:
                    ...
                    if 'tiers_id' in r and r['tiers_id']:
                        from apps.stocks.catalogue import tiers_disponible
                        try:
                            tiers = Tiers.objects.filter(id=UUID(str(r['tiers_id']))).first()
                        except (ValueError, TypeError, AttributeError):
                            tiers = None
                        if not tiers_disponible(tiers, e.societe_id):
                            return refus({'detail': 'Tiers de répartition indisponible dans cette société.'}, status=400)
                # 1ère répartition réutilise la ligne existante
                first = repartition[0]
                if not _analytique_compatible(ligne, first['compte_numero'], montants[0]):
                    return refus({'detail': 'Ajustez la ventilation analytique existante avant cet éclatement : elle dépasserait la première part ou porterait sur un compte hors charges/produits.'}, status=422)
                tiers_initial = ligne.tiers_id
                ligne.compte_numero = first["compte_numero"]
                ligne.montant_usd = round(float(first["montant"]), 2)
                ligne.tiers_id = first.get("tiers_id", tiers_initial)
                ...
                for r in repartition[1:]:
                    ...
                    LigneEcriture.objects.create(
                        ...
                        tiers_id=r.get("tiers_id", tiers_initial), montant_usd=round(float(r["montant"]), 2),
            ...
            for r in payload.get("reclassements", []):
                ...
                if ligne.compte_numero != r["compte_numero"]:
                    if not _analytique_compatible(ligne, r['compte_numero'], ligne.montant_usd):
                        return refus({'detail': 'Retirez la ventilation analytique existante avant de reclasser cette ligne hors charges/produits.'}, status=422)
