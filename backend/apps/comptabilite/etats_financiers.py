...
def bilan(societe_id, statut: str | None = None, annee: int | None = None) -> dict:
    ...
    # Le détail N−1 doit provenir de l'exercice précédent, pas de la copie des
    # soldes N utilisée en interne par _bilan_chiffres. Garder aussi les postes
    # sortis en N mais encore présents au comparatif.
    immo_n = {l['poste']: l for l in N['immo_lignes']}
    immo_n1 = {l['poste']: l for l in N1['immo_lignes']}
    immo_lignes = [
        {'poste': poste, 'intitule': (immo_n.get(poste) or immo_n1[poste])['intitule'],
         'montant': immo_n.get(poste, {}).get('montant', 0.0),
         'montant_n1': immo_n1.get(poste, {}).get('montant', 0.0)}
        for poste in sorted(immo_n.keys() | immo_n1.keys())
    ]
    return {
        "annee": annee,
        "actif": {
            "immobilise": {"total": N["immo"], "total_n1": N1["immo"], "lignes": immo_lignes},
