...
from apps.approbations.serializers import RequisitionSerializer
from apps.approbations.models import OrdreDepense
from apps.approbations.serializers import OrdreDepenseSerializer
from apps.tresorerie.models import Avance
from apps.approbations.serializers import AvanceSerializer
from apps.tresorerie.models import BlocageBeneficiaire
from apps.approbations.serializers import BlocageBeneficiaireSerializer
...
class RequisitionViewSet(MetierModelViewSet):
    """Ressource Requisition ; contrats HTTP et validations métier conservés."""
    queryset = Requisition.objects.none()
    serializer_class = RequisitionSerializer
    lookup_url_kwarg = 'requisition_id'

    def list(self, request):
        return self._traiter_requisitions(request)
    ...
    def _traiter_requisitions(self, request):
    ...
    def retrieve(self, request, requisition_id):
        ...
        return Response({
            "id": str(r.id), "societe_id": str(r.societe_id), "numero": r.numero, "objet": r.objet,
    ...
    def commentaires(self, request, requisition_id):
    ...
    def demander_precisions(self, request, requisition_id):
    ...
    @action(detail=True, methods=['post'])
    def repondre(self, request, requisition_id):
    ...
    @action(detail=True, methods=['post'])
    def valider_demande(self, request, requisition_id):
        req = Requisition.objects.filter(id=requisition_id).first()
        if not req:
            return refus({"detail": "Réquisition introuvable."}, status=404)
        roles = assert_acces_societe(request.user, req.societe_id)
...
class OrdreDepenseViewSet(MetierModelViewSet):
    """Ressource OrdreDepense ; contrats HTTP et validations métier conservés."""
    queryset = OrdreDepense.objects.none()
    serializer_class = OrdreDepenseSerializer
    lookup_url_kwarg = 'ordre_id'

    def list(self, request):
        return self._traiter_ordres_depense(request)
    ...
    @action(detail=True, methods=['post'])
    def valider(self, request, ordre_id):
    ...
    @action(detail=True, methods=['post'])
    @transaction.atomic
    def executer(self, request, ordre_id):
    ...
    @action(detail=True, methods=['get'])
    def bon_sortie(self, request, ordre_id):
        ...
        bons = BonReception.objects.filter(ordre_depense_id=odp.id)
        if request.query_params.get("bon_numero"):
            bons = bons.filter(numero=request.query_params["bon_numero"])
        brf = bons.order_by("-date_reception", "-numero").first()
        if not brf:
            return refus({"detail": "Aucun décaissement exécuté pour cet ordre."}, status=404)
        societe = Societe.objects.filter(id=odp.societe_id).first()
        benef = Tiers.objects.filter(id=brf.receveur_tiers_id).first()
        ...
        return Response({
            ...
            "devise": brf.devise, "montant": float(brf.montant),
            "montant_usd": float(brf.montant_usd),
...
def _resolve_article(societe: Societe, m: dict) -> Article | Response:
    ...
    if m.get("article_id"):
        a = Article.objects.filter(id=m["article_id"], societe_id=societe.id).first()
        if not a:
...
class AvanceViewSet(MetierModelViewSet):
    """Ressource Avance ; contrats HTTP et validations métier conservés."""
    queryset = Avance.objects.none()
    ...
    @action(detail=False, methods=['post'])
    def verifier_retards(self, request):
...
class BlocageViewSet(MetierModelViewSet):
    """Ressource Blocage ; contrats HTTP et validations métier conservés."""
    queryset = BlocageBeneficiaire.objects.none()
    serializer_class = BlocageBeneficiaireSerializer
    lookup_url_kwarg = 'blocage_id'

    @action(detail=True, methods=['post'])
    def lever(self, request, blocage_id):
...
class CentreApprobationViewSet(MetierViewSet):
    """Ressource CentreApprobation ; contrats HTTP et validations métier conservés."""

    @action(detail=False, methods=['get'])
    def centre_approbation(self, request):
        ...
        for sid, info in contexte.items():
            ...
            if "DFI" in mes_roles:
                for r in Requisition.objects.filter(societe_id=sid, statut="demande_validee"):
                    a_emettre.append({
                        "id": str(r.id), "numero": r.numero, "societe": info["code"], "societe_id": str(sid),
            ...
            if paliers_sf:
                for o in OrdreDepense.objects.filter(societe_id=sid, statut="a_valider"):
                    ...
                    if a_agir:
                        ordres.append({
                            "id": str(o.id), "numero": o.numero, "societe": info["code"], "societe_id": str(sid),
...
class DashboardViewSet(MetierViewSet):
    """Ressource Dashboard ; contrats HTTP et validations métier conservés."""
...
requisitions = RequisitionViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='requisition')
requisition_detail = RequisitionViewSet.as_view({'get': 'retrieve'}, http_method_names=['get', 'options'], detail=True, basename='requisition')
requisition_commentaires = RequisitionViewSet.as_view({'get': 'commentaires'}, http_method_names=['get', 'options'], detail=True, basename='requisition')
demander_precisions = RequisitionViewSet.as_view({'post': 'demander_precisions'}, http_method_names=['post', 'options'], detail=True, basename='requisition')
repondre = RequisitionViewSet.as_view({'post': 'repondre'}, http_method_names=['post', 'options'], detail=True, basename='requisition')
valider_demande = RequisitionViewSet.as_view({'post': 'valider_demande'}, http_method_names=['post', 'options'], detail=True, basename='requisition')
ordres_depense = OrdreDepenseViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='ordre_depense')
valider_sortie = OrdreDepenseViewSet.as_view({'post': 'valider'}, http_method_names=['post', 'options'], detail=True, basename='ordre_depense')
executer = OrdreDepenseViewSet.as_view({'post': 'executer'}, http_method_names=['post', 'options'], detail=True, basename='ordre_depense')
bon_sortie = OrdreDepenseViewSet.as_view({'get': 'bon_sortie'}, http_method_names=['get', 'options'], detail=True, basename='ordre_depense')
lister_avances = AvanceViewSet.as_view({'get': 'list'}, http_method_names=['get', 'options'], detail=False, basename='avance')
justifier = AvanceViewSet.as_view({'post': 'justifier'}, http_method_names=['post', 'options'], detail=False, basename='avance')
verifier_retards = AvanceViewSet.as_view({'post': 'verifier_retards'}, http_method_names=['post', 'options'], detail=False, basename='avance')
lever_blocage = BlocageViewSet.as_view({'post': 'lever'}, http_method_names=['post', 'options'], detail=True, basename='blocage')
centre_approbation = CentreApprobationViewSet.as_view({'get': 'centre_approbation'}, http_method_names=['get', 'options'], detail=False, basename='centre_approbation')
dashboard = DashboardViewSet.as_view({'get': 'dashboard'}, http_method_names=['get', 'options'], detail=False, basename='dashboard')
