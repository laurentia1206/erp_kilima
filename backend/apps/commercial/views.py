...
from apps.commercial.serializers import ArticleSerializer
from core.models import Tiers
from apps.commercial.serializers import TiersSerializer
from apps.commercial.models import Facture
from apps.commercial.serializers import FactureSerializer
from apps.commercial.models import Commande
from apps.commercial.serializers import CommandeSerializer
from apps.commercial.models import Reception
from apps.commercial.serializers import ReceptionSerializer
from apps.commercial.models import ListePrix
from apps.commercial.serializers import ListePrixSerializer
from apps.commercial.models import PointVente
from apps.commercial.serializers import PointVenteSerializer
...
from apps.tresorerie.views import _session_ouverte, _soldes
from apps.stocks.models import Article, Depot, MouvementStock
from apps.tresorerie.models import Avance, Caisse, CompteBancaire, Justification, MouvementCaisse, SessionCaisse
from apps.commercial.models import Commande, Devis, Facture, FraisFacture, FraisReception, LigneCommande, LigneFacture, LigneReception, ListePrix, PaiementFacture, PointVente, Reception, TarifArticle
from apps.transport.models import Course
from apps.comptabilite.models import LigneEcriture
from core.models import Societe, Tiers, Utilisateur
from core.views import _societe_param
from apps.stocks.catalogue import verifier_doublon, verrouiller_catalogue, tiers_visibles, tiers_disponible, normaliser
...
def _article_dict(a: Article) -> dict:
    return ArticleSerializer(a).data


# ── Tiers (clients / fournisseurs) ───────────────────────────────────
def _tiers_dict(t: Tiers) -> dict:
    return TiersSerializer(t).data
...
class TiersViewSet(MetierModelViewSet):
    ...
    lookup_url_kwarg = 'tiers_id'

    def list(self, request):
        return self._traiter_tiers_commercial(request)
    ...
    @transaction.atomic
    def _traiter_tiers_commercial(self, request):
        ...
        return Response(self.get_serializer(q.order_by("nom"), many=True).data)


    @transaction.atomic
    def partial_update(self, request, tiers_id):
    ...
    def get_queryset(self):
        ...
        _acces(self.request, sid, ROLES_POS)
        return tiers_visibles(sid)
...
class FactureViewSet(MetierModelViewSet):
    ...
    def list(self, request):
        return self._traiter_factures(request)
    ...
    def _traiter_factures(self, request):
        sid = _societe_param(request)
        _acces(request, sid)
        if request.method == "POST":
    ...
    def retrieve(self, request, facture_id):
...
class StockViewSet(MetierViewSet):
    """Ressource Stock ; contrats HTTP et validations métier conservés."""
...
class AchatsHistoriqueViewSet(MetierViewSet):
    """Ressource AchatsHistorique ; contrats HTTP et validations métier conservés."""

    @action(detail=False, methods=['get'])
    def achats_historique(self, request):
...
class VentesSyntheseViewSet(MetierViewSet):
    """Ressource VentesSynthese ; contrats HTTP et validations métier conservés."""

    @action(detail=False, methods=['get'])
    def ventes_synthese(self, request):
...
def _commande_dict(c: Commande) -> dict:
    ...
    return {"id": str(c.id), "numero": c.numero, "tiers": tiers.nom if tiers else None,
            ...
            "transporteur_societe_id": str(c.transporteur_societe_id) if c.transporteur_societe_id else None,
...
class CommandeViewSet(MetierModelViewSet):
    ...
    def list(self, request):
        return self._traiter_commandes(request)
    ...
    def _traiter_commandes(self, request):
        sid = _societe_param(request)
        _acces(request, sid)
        if request.method == "POST":
            payload = request.data or {}
    ...
    def retrieve(self, request, commande_id):
        return self._traiter_commande_detail(request, commande_id)


    def update(self, request, commande_id):
        return self._traiter_commande_detail(request, commande_id)


    def _traiter_commande_detail(self, request, commande_id):
    ...
    def annuler(self, request, commande_id):
        c = Commande.objects.filter(id=commande_id).first()
        if not c:
            return refus({"detail": "Commande introuvable."}, status=404)
        _acces(request, c.societe_id)
...
class ReceptionViewSet(MetierModelViewSet):
    ...
    queryset = Reception.objects.none()
    ...
    @action(detail=True, methods=['post'])
    def facturer(self, request, reception_id):
...
class ListePrixViewSet(MetierModelViewSet):
    """Ressource ListePrix ; contrats HTTP et validations métier conservés."""
    queryset = ListePrix.objects.none()
    serializer_class = ListePrixSerializer
    lookup_url_kwarg = 'liste_id'

    def list(self, request):
        return self._traiter_listes_prix(request)
    ...
    def _traiter_listes_prix(self, request):
        sid = _societe_param(request)
        _acces(request, sid)
        if request.method == "POST":
            payload = request.data or {}
    ...
    def get_tarifs(self, request, liste_id):
        return self._traiter_tarifs_liste(request, liste_id)


    @action(detail=True, methods=['post'])
    def post_tarifs(self, request, liste_id):
        return self._traiter_tarifs_liste(request, liste_id)


    def _traiter_tarifs_liste(self, request, liste_id):
...
class PointVenteViewSet(MetierModelViewSet):
    """Ressource PointVente ; contrats HTTP et validations métier conservés."""
    queryset = PointVente.objects.none()
    serializer_class = PointVenteSerializer
    lookup_url_kwarg = 'pv_id'

    def list(self, request):
        return self._traiter_points_vente(request)
    ...
    def partial_update(self, request, pv_id):
...
class ArticleViewSet(MetierModelViewSet):
    """Ressource Article ; contrats HTTP et validations métier conservés."""
    queryset = Article.objects.all()
    serializer_class = ArticleSerializer
    lookup_url_kwarg = 'article_id'

    def list(self, request):
        return self._traiter_articles(request)
    ...
    def _traiter_articles(self, request):
        ...
        arts = list(self.get_queryset().order_by("code"))
        q = request.query_params.get("q")
        if q:
            ql = q.lower()
            arts = [a for a in arts if ql in a.code.lower() or ql in a.designation.lower()]
        return Response(self.get_serializer(arts, many=True).data)


    @transaction.atomic
    def partial_update(self, request, article_id):
    ...
    @action(detail=True, methods=['get'])
    def prix(self, request, article_id):
    ...
    def get_queryset(self):
        sid = _societe_param(self.request)
        _acces(self.request, sid, ROLES_POS)
        return Article.objects.filter(societe_id=sid)
...
class PointDeVenteViewSet(MetierViewSet):
    """Ressource PointDeVente ; contrats HTTP et validations métier conservés."""
    lookup_url_kwarg = 'facture_id'
            ...
            if chambre_usd:
                encaissements.append({"compte": compte_par_mode["chambre"],
                                      ...
                                      "libelle": f"Sur chambre POS {numero} — "
                                                 ...
                                                 f"({tiers.nom})"})
                from apps.hotel.models import LigneSejour
    ...
    @action(detail=True, methods=['get'])
    def ticket(self, request, facture_id):
        f = Facture.objects.filter(id=facture_id).first()
...
articles = ArticleViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='article')
maj_article = ArticleViewSet.as_view({'patch': 'partial_update'}, http_method_names=['patch', 'options'], detail=True, basename='article')
tiers_commercial = TiersViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='tiers')
maj_tiers = TiersViewSet.as_view({'patch': 'partial_update'}, http_method_names=['patch', 'options'], detail=True, basename='tiers')
factures = FactureViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='facture')
detail_facture = FactureViewSet.as_view({'get': 'retrieve'}, http_method_names=['get', 'options'], detail=True, basename='facture')
etat_stock = StockViewSet.as_view({'get': 'etat_stock'}, http_method_names=['get', 'options'], detail=False, basename='stock')
mouvements_stock = StockViewSet.as_view({'get': 'mouvements'}, http_method_names=['get', 'options'], detail=False, basename='stock')
achats_historique = AchatsHistoriqueViewSet.as_view({'get': 'achats_historique'}, http_method_names=['get', 'options'], detail=False, basename='achats_historique')
ventes_synthese = VentesSyntheseViewSet.as_view({'get': 'ventes_synthese'}, http_method_names=['get', 'options'], detail=False, basename='ventes_synthese')
commandes = CommandeViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='commande')
commande_detail = CommandeViewSet.as_view({'get': 'retrieve', 'put': 'update'}, http_method_names=['get', 'put', 'options'], detail=True, basename='commande')
annuler_commande = CommandeViewSet.as_view({'post': 'annuler'}, http_method_names=['post', 'options'], detail=True, basename='commande')
receptionner = CommandeViewSet.as_view({'post': 'receptionner'}, http_method_names=['post', 'options'], detail=True, basename='commande')
lister_receptions = ReceptionViewSet.as_view({'get': 'list'}, http_method_names=['get', 'options'], detail=False, basename='reception')
detail_reception = ReceptionViewSet.as_view({'get': 'retrieve'}, http_method_names=['get', 'options'], detail=True, basename='reception')
facturer_reception = ReceptionViewSet.as_view({'post': 'facturer'}, http_method_names=['post', 'options'], detail=True, basename='reception')
listes_prix = ListePrixViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='liste_prix')
tarifs_liste = ListePrixViewSet.as_view({'get': 'get_tarifs', 'post': 'post_tarifs'}, http_method_names=['get', 'post', 'options'], detail=True, basename='liste_prix')
points_vente = PointVenteViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='point_vente')
maj_point_vente = PointVenteViewSet.as_view({'patch': 'partial_update'}, http_method_names=['patch', 'options'], detail=True, basename='point_vente')
prix_article = ArticleViewSet.as_view({'get': 'prix'}, http_method_names=['get', 'options'], detail=True, basename='article')
pos_contexte = PointDeVenteViewSet.as_view({'get': 'contexte'}, http_method_names=['get', 'options'], detail=False, basename='point_de_vente')
pos_vente = PointDeVenteViewSet.as_view({'post': 'vente'}, http_method_names=['post', 'options'], detail=False, basename='point_de_vente')
pos_tickets = PointDeVenteViewSet.as_view({'get': 'tickets'}, http_method_names=['get', 'options'], detail=False, basename='point_de_vente')
pos_ticket = PointDeVenteViewSet.as_view({'get': 'ticket'}, http_method_names=['get', 'options'], detail=True, basename='point_de_vente')
pos_retour = PointDeVenteViewSet.as_view({'post': 'retour'}, http_method_names=['post', 'options'], detail=False, basename='point_de_vente')
pos_rapport = PointDeVenteViewSet.as_view({'get': 'rapport'}, http_method_names=['get', 'options'], detail=False, basename='point_de_vente')
