...
class AuditViewSet(ReadOnlyModelViewSet):
    ...
    def scoped(self):
        ...
        if est_super_admin(request.user):
            ...
            self.company=None
            q=AuditLog.objects.filter(table_cible__in=('utilisateur','utilisateur_societe','role','permission_module','super_administrateur','authentification','journal_securite'))
            # Une connexion est tracée par société pour le DFI. Dans la vue système,
            # la même requête ne doit pas être comptée autant de fois que de sociétés.
            first=AuditLog.objects.filter(table_cible='authentification',requete_id__isnull=False).order_by().values('requete_id','action','utilisateur_id').annotate(first_id=Min('id')).values('first_id')
            return q.filter(~Q(table_cible='authentification') | Q(requete_id__isnull=True) | Q(id__in=first))
    ...
    def get_queryset(self):
        ...
        if p.get('adresse_ip'):
            try:address=str(ipaddress.ip_address(p['adresse_ip']))
            except ValueError:raise ValidationError('Adresse IP invalide.')
            q=q.filter(adresse_ip=address)
        if p.get('attention'):
            if p['attention']!='1':raise ValidationError('Filtre de sécurité invalide.')
            q=q.filter(action__in=('CONNEXION_ECHEC','ACCES_REFUSE','MOT_DE_PASSE_ECHEC','ERREUR_SERVEUR'))
        dates={}
        for key in ('du','au'):
            if p.get(key):
        ...
        if p.get('q'):
            term=p['q'][:100]
            q=q.filter(Q(action__icontains=term)|Q(table_cible__icontains=term)|Q(enregistrement_id__icontains=term)|Q(adresse_ip__icontains=term))
    ...
    def list(self,request):
        ...
        return Response({'resultats':[public_record(r,users) for r in rows], 'total':total,'page':page,'taille_page':size,'borne':int(request.query_params.get('borne') or bound),
            ...
            'actions':list(scope.order_by('action').values_list('action',flat=True).distinct()),
            'indicateurs':{'connexions':q.filter(action='CONNEXION_REUSSIE').count(),
                'echecs':q.filter(action__in=('CONNEXION_ECHEC','MOT_DE_PASSE_ECHEC')).count(),
                'refus':q.filter(action='ACCES_REFUSE').count(),
                'erreurs':q.filter(action='ERREUR_SERVEUR').count(),
                'habilitations':q.filter(table_cible__in=('permission_module','utilisateur_societe','role','super_administrateur')).count()},
    ...
    def retrieve(self,request,*args,**kwargs):
        ...
        result=public_record(obj,users,True)
        from .permissions import est_super_admin
        if est_super_admin(request.user):
            append_event(request.user.id,'CONSULTATION_TRACE','journal_securite',new={'trace_id':obj.id},categorie='consultation',module='audit')
    ...
    def exporter(self,request):
        ...
        from .permissions import est_super_admin
        append_event(request.user.id,'EXPORT_AUDIT','journal_securite' if est_super_admin(request.user) else 'audit_log',new={'format':fmt,'nombre':len(rows),'filtres':filters},societe_id=self.company.id if self.company else None,categorie='consultation',module='audit')
