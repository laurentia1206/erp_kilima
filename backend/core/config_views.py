...
UPLOADS = dj_settings.UPLOADS_DIR
MAX_OCTETS = dj_settings.MAX_UPLOAD_MB * 1024 * 1024


def _est_admin(user) -> bool:
    from .permissions import est_super_admin
    if est_super_admin(user): return True
    ...
    def _traiter_societes_config(self, request):
        ...
        if est_super_admin(request.user):
            if request.method!='GET':
                return refus({'detail':'La gestion des sociétés est interdite au super administrateur.'},status=403)
            return Response([{'id':str(s.id),'code':s.code,'nom':s.nom,'actif':bool(s.actif)} for s in Societe.objects.order_by('code')])
        if not _est_admin(request.user):
            return refus({"detail": "Accès paramétrage réservé au DFI / Président / "
                                    "Admin."}, status=403)
        if request.method == "POST":
        ...
        if request.method == "DELETE":
            ...
            with transaction.atomic():
                ...
                with connection.cursor() as cur:
                    ...
                    tables = [t for (t,) in cur.fetchall() if t not in ("societe", "audit_log")]
                    for t in tables:
...
class UtilisateurViewSet(MetierModelViewSet):
    ...
    def partial_update(self, request, utilisateur_id):
        if not _est_admin(request.user):
            return refus({"detail": "Accès paramétrage réservé au DFI / Président / "
                                    "Admin."}, status=403)
        from .admin_views import verrou_admin
        verrou_admin()
        ...
        from .permissions import est_super_admin
        from .models import SuperAdministrateur
        if SuperAdministrateur.objects.filter(utilisateur=u).exists():
            if not est_super_admin(request.user):
                return refus({'detail':'Ce compte est protégé. Intervention du super administrateur requise.'},status=403)
            if 'actif' in request.data and request.data['actif'] is not True:
                return refus({'detail':'Retirez le statut de super administrateur avant de désactiver ce compte.'},status=400)
        payload = request.data or {}
        if payload.get("nom"):
        ...
        if payload.get("password"):
            ...
            u.password_hash = bcrypt.hashpw(payload["password"].encode("utf-8")[:72],
                                            bcrypt.gensalt()).decode("utf-8")
        u.save()
        if payload.get('password') and est_super_admin(u):
            from .models import SuperAdministrateur
            SuperAdministrateur.objects.filter(utilisateur=u).update(changer_mot_de_passe=True)
    ...
    def _traiter_pieces_jointes(self, request):
        if request.method == "POST":
            ...
            if f.size > MAX_OCTETS:
                return refus({"detail": f"Fichier trop volumineux (max {dj_settings.MAX_UPLOAD_MB} Mo)."}, status=413)
            data = f.read()
