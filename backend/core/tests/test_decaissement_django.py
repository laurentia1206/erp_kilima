...
from core.models import (Article, Avance, BonReception, Caisse, Ecriture, LigneEcriture, MouvementCaisse, OrdreDepense,
...
class CircuitDecaissementTests(TestCase):
    ...
    def test_bon_restitue_le_paiement_partiel_et_son_receveur_historique(self):
        ordre = self.ordre()
        for role in ["DG", "ADMIN", "PRESIDENT"]:
            self.post(role, f"ordres-depense/{ordre['id']}/valider", {"decision": "valide"})
        self.post("CAISSIER_CENTRAL", f"caisse/{self.caisse.id}/ouvrir", {"fond_initial_usd": 10000}, 201)
        premier = self.post("CAISSIER_CENTRAL", f"ordres-depense/{ordre['id']}/executer",
            {"caisse_id": str(self.caisse.id), "montant": 1000})
        autre = Tiers.objects.create(societe_id=self.soc.id, code="AUTRE", nom="Autre agent", type="agent")
        second = self.post("CAISSIER_CENTRAL", f"ordres-depense/{ordre['id']}/executer",
            {"caisse_id": str(self.caisse.id), "montant": 500, "beneficiaire_tiers_id": str(autre.id)})
        url = f"/api/ordres-depense/{ordre['id']}/bon-sortie"
        original = self.clients["DFI"].get(url, {"bon_numero": premier["bon_reception"]})
        self.assertEqual(original.status_code, 200, original.content)
        self.assertEqual(original.json()["montant"], 1000)
        self.assertEqual(original.json()["montant_usd"], 1000)
        self.assertEqual(original.json()["beneficiaire"], self.beneficiaire.nom)
        dernier = self.clients["DFI"].get(url)
        self.assertEqual(dernier.json()["bon_numero"], second["bon_reception"])
        self.assertEqual(dernier.json()["montant"], 500)
        self.assertEqual(dernier.json()["beneficiaire"], autre.nom)
        with transaction.atomic():
            inconnu = self.clients["DFI"].get(url, {"bon_numero": "autre-ordre"})
        self.assertEqual(inconnu.status_code, 404)

    def test_justification_refuse_article_autre_societe_sans_mutation(self):
        ordre = self.ordre()
        for role in ["DG", "ADMIN", "PRESIDENT"]:
            self.post(role, f"ordres-depense/{ordre['id']}/valider", {"decision": "valide"})
        self.post("CAISSIER_CENTRAL", f"caisse/{self.caisse.id}/ouvrir", {"fond_initial_usd": 10000}, 201)
        self.post("CAISSIER_CENTRAL", f"ordres-depense/{ordre['id']}/executer", {"caisse_id": str(self.caisse.id)})
        avance = Avance.objects.get()
        autre = Societe.objects.create(code="AUTRE", nom="Autre société")
        article = Article.objects.create(societe_id=autre.id, code="ART", designation="Privé")
        avant = Ecriture.objects.count()
        self.post("COMPTABLE", "avances/justifier", {"avance_id": str(avance.id), "marchandises": [{"article_id": str(article.id), "qte": 1, "prix_unitaire": 5000, "taux_tva": 0}]}, 400)
        avance.refresh_from_db()
        self.assertEqual(avance.statut, "a_justifier")
        self.assertEqual(Ecriture.objects.count(), avant)

    def test_approbation_indique_societe_document(self):
        req = self.demande()
        detail = self.clients["DFI"].get(f"/api/requisitions/{req['id']}/detail").json()
        self.assertEqual(detail["societe_id"], str(self.soc.id))
        centre = self.clients["DG"].get("/api/approbations").json()
        self.assertEqual(centre["requisitions"][0]["societe_id"], str(self.soc.id))
