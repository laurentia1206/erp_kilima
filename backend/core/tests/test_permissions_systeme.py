...
class PermissionsSystemeTests(TestCase):
    ...
    def test_security_summary_deduplicates_company_events_and_filters_ip(self):
        from core.audit import append_event
        import uuid
        request_id=uuid.uuid4()
        for company in (self.a,self.b):
            AuditLog.objects.create(utilisateur_id=self.user.id,action='CONNEXION_ECHEC',table_cible='authentification',societe_id=company.id,categorie='securite',module='securite',requete_id=request_id,adresse_ip='192.0.2.10',nouvelle_valeur={'raison':'identifiants_invalides'})
        append_event(self.user.id,'CONNEXION_REUSSIE','authentification',categorie='securite')
        url='/api/audit/journal?portee=global&attention=1&adresse_ip=192.0.2.10'
        r=self.client.get(url)
        self.assertEqual(r.status_code,200,r.data)
        self.assertEqual(r.data['total'],1)
        self.assertEqual(r.data['indicateurs']['echecs'],1)
        self.assertEqual(r.data['indicateurs']['connexions'],0)
        self.assertIn('CONNEXION_ECHEC',r.data['actions'])
        for bad in ('adresse_ip=invalid','attention=2'):
            self.assertEqual(self.client.get('/api/audit/journal?portee=global&'+bad).status_code,400)
        # Le DFI conserve sa copie de l'événement pour sa propre société.
        self.client.force_authenticate(self.user)
        self.assertEqual(self.client.get(f'/api/audit/journal?societe_id={self.a.id}&action=CONNEXION_ECHEC').data['total'],1)

    def test_security_log_consultation_export_and_business_isolation(self):
        from core.audit import append_event
        row=append_event(self.user.id,'CONNEXION_REUSSIE','authentification',new={'password':'never-visible','token':'never-visible'},categorie='securite')
        r=self.client.get(f'/api/audit/journal/{row.id}?portee=global')
        self.assertEqual(r.status_code,200)
        self.assertNotIn('never-visible',str(r.data))
        self.assertTrue(AuditLog.objects.filter(table_cible='journal_securite',action='CONSULTATION_TRACE').exists())
        secret=append_event(self.user.id,'EXPORT_AUDIT','audit_log',new={'secret_societe':'private-business'},societe_id=self.a.id)
        for fmt in ('xlsx','pdf'):
            r=self.client.get('/api/audit/export?portee=global&action=CONNEXION_REUSSIE&format_export='+fmt)
            self.assertEqual(r.status_code,200)
        r=self.client.get('/api/audit/journal?portee=global&action=EXPORT_AUDIT')
        self.assertEqual(r.data['total'],2)
        self.assertTrue(all(x['table']=='journal_securite' for x in r.data['resultats']))
        self.assertEqual(self.client.get(f'/api/audit/journal/{secret.id}?portee=global').status_code,404)

    def test_logout_and_password_security_events_without_secrets(self):
        r=self.client.post('/api/auth/logout',{},format='json')
        self.assertEqual(r.status_code,200)
        self.assertTrue(AuditLog.objects.filter(action='DECONNEXION',utilisateur_id=self.root.id).exists())
        r=self.client.post('/api/systeme/mot-de-passe',{'ancien':'incorrect-secret','nouveau':'new-secret-password'},format='json')
        self.assertEqual(r.status_code,400)
        self.assertTrue(AuditLog.objects.filter(action='MOT_DE_PASSE_ECHEC').exists())
        r=self.client.post('/api/systeme/mot-de-passe',{'ancien':'initial-password','nouveau':'new-secret-password'},format='json')
        self.assertEqual(r.status_code,200)
        self.assertTrue(AuditLog.objects.filter(action='MOT_DE_PASSE_MODIFIE').exists())
        logs=str(list(AuditLog.objects.filter(categorie='securite').values('nouvelle_valeur')))
        for secret in ('incorrect-secret','new-secret-password','initial-password'):self.assertNotIn(secret,logs)
        client=APIClient()
        self.assertEqual(client.post('/api/auth/logout',{},format='json').status_code,401)

    def test_server_failure_records_only_safe_metadata(self):
        from django.test import RequestFactory
        from django.http import HttpResponse
        from core.audit_context import AuditContextMiddleware
        request=RequestFactory().get('/api/example?token=never-log-this',REMOTE_ADDR='192.0.2.22')
        request.user=self.root
        response=AuditContextMiddleware(lambda r:HttpResponse('private-business-detail',status=500))(request)
        self.assertEqual(response.status_code,500)
        row=AuditLog.objects.filter(action='ERREUR_SERVEUR').latest('id')
        self.assertEqual(row.nouvelle_valeur,{'statut':500})
        self.assertEqual(row.chemin,'/api/example')
        self.assertEqual(row.adresse_ip,'192.0.2.22')
