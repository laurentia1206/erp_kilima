...
class SuperAdministrateur(models.Model):
    utilisateur = models.OneToOneField(Utilisateur, primary_key=True, on_delete=models.PROTECT)
    created_at = models.DateTimeField(auto_now_add=True)
...
class AuditLogQuerySet(models.QuerySet):
    def update(self, **kwargs):
        raise ValueError("Le journal d’audit est en lecture seule.")

    def delete(self):
        raise ValueError("Le journal d’audit ne peut pas être supprimé.")
...
class AuditLog(models.Model):
    ...
    societe_id = models.UUIDField(null=True, blank=True)
    module = models.CharField(max_length=32, default='core')
    categorie = models.CharField(max_length=16, default='metier')
    champs_modifies = models.JSONField(default=list)
    requete_id = models.UUIDField(null=True, blank=True)
    methode = models.CharField(max_length=12, blank=True, default='')
    chemin = models.CharField(max_length=255, blank=True, default='')
    origine = models.CharField(max_length=16, default='application')
    ...
    class Meta:
        ...
        db_table = "audit_log"
        indexes = [
            models.Index(fields=['societe_id', '-id'], name='audit_societe_id_idx'),
            models.Index(fields=['utilisateur_id', '-id'], name='audit_user_id_idx'),
            models.Index(fields=['horodatage'], name='audit_date_idx'),
            models.Index(fields=['requete_id'], name='audit_requete_idx'),
            models.Index(fields=['table_cible', 'enregistrement_id'], name='audit_document_idx'),
        ]
