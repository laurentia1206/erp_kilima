...
from .models import (Societe, Utilisateur, UtilisateurSociete, Tiers, RHEquipe, RHHoraire,
    RHAgent, RHContrat, RHEvenement, RHDocument, RHPointage, RHDemande, RHDette, RHSimulation)
...
@api_view(['POST'])
@transaction.atomic
def pointage_decision(request,pointage_id):
...
@api_view(['POST'])
@transaction.atomic
def documents(request,agent_id):
    sid,roles=contexte(request);a=agent_requis(sid,agent_id)
