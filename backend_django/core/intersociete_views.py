...
from . import comptabilite, intersociete_lib, services
from .erreurs import refus
from .auth import assert_acces_societe, assert_role
from .caisse_views import _session_ouverte
from .commercial_views import _reglement_facture
from .intersociete_lib import (_mapping_lignes_po, _recu_cumule_po, reception_po_resume,
                               tiers_reciproque)
from .models import (Article, Caisse, Camion, Commande, Course, Devis, Facture,
                     LigneCommande, LigneDevis, LigneEcriture, LigneLivraison,
                     LigneReceptionInter, Livraison, MouvementCaisse, MouvementStock,
                     PaiementFacture, ReceptionInter, Societe, Tiers,
                     UtilisateurSociete)
...
@api_view(["POST"])
def receptionner_po(request, commande_id):
...
@api_view(["GET", "DELETE"])
def reception_detail_ou_annulation(request, reception_id):
...
@api_view(["GET"])
def receptions_intersociete(request):
...
@api_view(["POST"])
def confirmer_reception(request, reception_id):
...
@api_view(["GET"])
def factures_intragroupe(request):
...
@api_view(["POST"])
def regler_intersociete(request, facture_id):
