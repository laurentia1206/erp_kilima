...
@api_view(["PATCH", "DELETE"])
def societe_detail(request, societe_id):
    if not _est_admin(request.user):
        return refus({"detail": "Accès paramétrage réservé au DFI / Président / "
                                "Admin."}, status=403)
...
@api_view(["PATCH"])
def maj_utilisateur(request, utilisateur_id):
    if not _est_admin(request.user):
        return refus({"detail": "Accès paramétrage réservé au DFI / Président / "
                                "Admin."}, status=403)
...
@api_view(["GET"])
def telecharger_piece(request, piece_id):
