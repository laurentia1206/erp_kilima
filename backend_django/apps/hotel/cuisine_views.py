...
from apps.hotel.serializers import FicheTechniqueSerializer
from apps.hotel.models import ConsommationCuisine
from apps.hotel.serializers import ConsommationCuisineSerializer
...
from apps.stocks.models import Article, Depot, InventaireDepot, LigneInventaireDepot
from apps.hotel.models import ConsommationCuisine, FicheTechnique, LigneConsommationCuisine, LigneFicheTechnique
from apps.commercial.models import Facture, LigneFacture
from core.models import Parametre, Societe
...
class FicheTechniqueViewSet(MetierModelViewSet):
    """Ressource FicheTechnique ; contrats HTTP et validations métier conservés."""
    queryset = FicheTechnique.objects.none()
    serializer_class = FicheTechniqueSerializer
    lookup_url_kwarg = 'fiche_id'

    def list(self, request):
        return self._traiter_fiches(request)
    ...
    def _traiter_fiches(self, request):
    ...
    def destroy(self, request, fiche_id):
...
class ConsommationViewSet(MetierModelViewSet):
    """Ressource Consommation ; contrats HTTP et validations métier conservés."""
    queryset = ConsommationCuisine.objects.none()
    serializer_class = ConsommationCuisineSerializer

    def list(self, request):
        return self._traiter_consommations(request)
    ...
    def _traiter_consommations(self, request):
        sid = _societe_param(request)
        _acces(request, sid)
        seuil = _seuil(sid)
        if request.method == "POST":
...
class CuisineViewSet(MetierViewSet):
    """Ressource Cuisine ; contrats HTTP et validations métier conservés."""

    @action(detail=False, methods=['get'])
    def get_config(self, request):
        return self._traiter_config_cuisine(request)
    ...
    def _traiter_config_cuisine(self, request):
        sid = _societe_param(request)
...
config_cuisine = CuisineViewSet.as_view({'get': 'get_config', 'post': 'post_config'}, http_method_names=['get', 'post', 'options'], detail=False, basename='cuisine')
fiches = FicheTechniqueViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='fiche_technique')
supprimer_fiche = FicheTechniqueViewSet.as_view({'delete': 'destroy'}, http_method_names=['delete', 'options'], detail=True, basename='fiche_technique')
consommations = ConsommationViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='consommation')
rapport_cuisine = CuisineViewSet.as_view({'get': 'rapport'}, http_method_names=['get', 'options'], detail=False, basename='cuisine')
