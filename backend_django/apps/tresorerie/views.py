...
from apps.tresorerie.serializers import CaisseSerializer
from apps.tresorerie.models import SessionCaisse
from apps.tresorerie.serializers import SessionCaisseSerializer
from apps.tresorerie.models import MouvementCaisse
from apps.tresorerie.serializers import MouvementCaisseSerializer
from apps.tresorerie.models import Transfert
from apps.tresorerie.serializers import TransfertSerializer
...
class CaisseViewSet(MetierModelViewSet):
    """Ressource Caisse ; contrats HTTP et validations métier conservés."""
    queryset = Caisse.objects.none()
    serializer_class = CaisseSerializer
    lookup_url_kwarg = 'caisse_id'

    def list(self, request):
        return self._traiter_caisses(request)
    ...
    def _traiter_caisses(self, request):
    ...
    def partial_update(self, request, caisse_id):
    ...
    def ouvrir(self, request, caisse_id):
    ...
    def session(self, request, caisse_id):
        caisse, _ = _caisse_ou_404(request, caisse_id)
        if not caisse:
            return refus({"detail": "Caisse introuvable."}, status=404)
    ...
    @action(detail=True, methods=['post'])
    def mouvement(self, request, caisse_id):
        caisse, _ = _caisse_ou_404(request, caisse_id)
        if not caisse:
            return refus({"detail": "Caisse introuvable."}, status=404)
        sess = _session_ouverte(caisse_id)
    ...
    @action(detail=True, methods=['post'])
    def operation(self, request, caisse_id):
    ...
    @action(detail=True, methods=['get'])
    def journal(self, request, caisse_id):
    ...
    @action(detail=True, methods=['post'])
    def cloturer(self, request, caisse_id):
...
class SessionCaisseViewSet(MetierModelViewSet):
    """Ressource SessionCaisse ; contrats HTTP et validations métier conservés."""
    queryset = SessionCaisse.objects.none()
    serializer_class = SessionCaisseSerializer
    lookup_url_kwarg = 'session_id'

    @action(detail=True, methods=['get'])
    def rapport_z(self, request, session_id):
...
class MouvementCaisseViewSet(MetierModelViewSet):
    """Ressource MouvementCaisse ; contrats HTTP et validations métier conservés."""
    queryset = MouvementCaisse.objects.none()
    serializer_class = MouvementCaisseSerializer
    lookup_url_kwarg = 'mvt_id'

    @action(detail=True, methods=['get'])
    def bon(self, request, mvt_id):
...
class JournalCaisseViewSet(MetierViewSet):
    """Ressource JournalCaisse ; contrats HTTP et validations métier conservés."""
...
class TransfertViewSet(MetierModelViewSet):
    """Ressource Transfert ; contrats HTTP et validations métier conservés."""
    queryset = Transfert.objects.none()
    serializer_class = TransfertSerializer
    lookup_url_kwarg = 'transfert_id'
    ...
    @action(detail=True, methods=['post'])
    def valider(self, request, transfert_id):
    ...
    @action(detail=True, methods=['post'])
    def rejeter(self, request, transfert_id):
        t = Transfert.objects.filter(id=transfert_id).first()
        if not t:
            return refus({"detail": "Transfert introuvable."}, status=404)
        roles = assert_acces_societe(request.user, t.societe_id)
...
caisses = CaisseViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='caisse')
maj_caisse = CaisseViewSet.as_view({'patch': 'partial_update'}, http_method_names=['patch', 'options'], detail=True, basename='caisse')
ouvrir = CaisseViewSet.as_view({'post': 'ouvrir'}, http_method_names=['post', 'options'], detail=True, basename='caisse')
session_courante = CaisseViewSet.as_view({'get': 'session'}, http_method_names=['get', 'options'], detail=True, basename='caisse')
mouvement = CaisseViewSet.as_view({'post': 'mouvement'}, http_method_names=['post', 'options'], detail=True, basename='caisse')
operation = CaisseViewSet.as_view({'post': 'operation'}, http_method_names=['post', 'options'], detail=True, basename='caisse')
journal_session = CaisseViewSet.as_view({'get': 'journal'}, http_method_names=['get', 'options'], detail=True, basename='caisse')
cloturer = CaisseViewSet.as_view({'post': 'cloturer'}, http_method_names=['post', 'options'], detail=True, basename='caisse')
rapport_z = SessionCaisseViewSet.as_view({'get': 'rapport_z'}, http_method_names=['get', 'options'], detail=True, basename='session_caisse')
bon_mouvement = MouvementCaisseViewSet.as_view({'get': 'bon'}, http_method_names=['get', 'options'], detail=True, basename='mouvement_caisse')
caisse_journal = JournalCaisseViewSet.as_view({'get': 'caisse_journal'}, http_method_names=['get', 'options'], detail=False, basename='journal_caisse')
transferts = TransfertViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='transfert')
valider_transfert = TransfertViewSet.as_view({'post': 'valider'}, http_method_names=['post', 'options'], detail=True, basename='transfert')
rejeter_transfert = TransfertViewSet.as_view({'post': 'rejeter'}, http_method_names=['post', 'options'], detail=True, basename='transfert')
