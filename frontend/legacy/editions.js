  ...
  async function afficher(original,sid=currentSocieteId){
    try{
      ...
      root.querySelector('[data-ed-print]').onclick=()=>{
        ...
        w.document.write(`<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>${esc(spec.titre)}</title><link rel="stylesheet" href="${window.KilimaNext ? "/legacy/editions.css" : "/static/editions.css"}?v=1"><style>@page{size:A4 ${landscape?'landscape':'portrait'}}</style></head><body>${content}</body></html>`);w.onload=()=>{w.focus();w.print();};w.document.close();
      };
