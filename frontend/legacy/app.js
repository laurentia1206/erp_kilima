...
function logout() {
  ...
  document.querySelectorAll(".view:not([data-react-owned])").forEach((v) => v.replaceChildren());
  $("#password").value = "";
  setMobileMenu(false);
  if (window.KilimaNext) window.KilimaNext.publish();
...
async function boot() {
  ...
  renderSidebar();
  if (!window.KilimaNext) { $("#login").classList.add("hidden"); $("#app").classList.remove("hidden"); }
  ...
  else if (window.KilimaNext?.initialView()) go(window.KilimaNext.initialView());
...
function renderSidebar() {
  if (window.KilimaNext) { window.KilimaNext.publish(); return; }
  let h = "";
...
async function go(view, groupePrefere) {
  if(window.KilimaNext?.canLeave && !window.KilimaNext.canLeave())return;
  ...
  if (window.KilimaNext && !visibleModules().some(g=>g.items.some(i=>i.v===view))) { toast('Ce module ne fait pas partie de vos accès.','ko'); return; }
  ...
  if(window.KilimaNext?.owns?.(view))return;
...
if (!window.KilimaNext) initWorkspace();
ModuleUX.init();
Catalogue.init();
