...
(() => {
  ...
  function snapshot(extra={}) {
    const groups = me ? visibleModules().map(group => ({
      name:group.g, items:group.items.map(item=>({view:item.v, label:me.super_administrateur&&item.v==='administration'?'Utilisateurs & rôles':me.super_administrateur&&item.v==='audit'?'Journal des utilisateurs':item.l, icon:item.i, badge:item.badge})),
    })) : [];
    const title = me?.super_administrateur&&activeView==='administration'?'Utilisateurs & rôles':me?.super_administrateur&&activeView==='audit'?'Journal des utilisateurs':TITLES[activeView]?.[0] || 'Accueil';
    return { user:me, companies:societes, companyId:currentSocieteId, view:activeView, title,
  ...
  for(const view of ['accueil','pilotage','audit','stock','taux','articles','hotel-chambres'])if(view==='accueil'||window.KilimaNext.owns?.(view))RENDER[view]=()=>{};
  const bridge={
    snapshot, api,
    ...
    async openTask(view,type,documentId){
      await go(view);
      if(activeView===view&&['req_validation','req_precision','ordre_emission'].includes(type))await detailsRequisition(documentId);
    },
    async restore(){
    ...
    logout(){if(!window.KilimaNext.canLeave())return;logout();history.replaceState(null,'','/');document.title='ERP KILIMA HOLDINGS';},
    ...
    async selectCompany(id){
      ...
      currentSocieteId=id;localStorage.setItem('kh_societe_'+me.id,id);
