...
import { auditActionLabel, attentionActions, auditQuery, assertAuditCapabilities } from '../../lib/audit-data';
...
interface Filters extends Record<string, string> { action: string; adresse_ip: string; attention: string; portee: string; du: string; au: string; utilisateur_id: string; module: string; categorie: string; q: string; requete_id: string }
...
interface Journal { resultats: RecordRow[]; total: number; page: number; taille_page: number; borne: number; utilisateurs: { id: string; nom: string }[]; modules: string[]; actions: string[]; indicateurs: { connexions: number; echecs: number; refus: number; erreurs: number; habilitations: number } }
const initial = (global: boolean): Filters => { const before = new Date(); before.setDate(before.getDate() - 7); return { portee: global ? 'global' : 'societe', du: before.toLocaleDateString('fr-CA', { timeZone: 'Africa/Lubumbashi' }), au: today(), utilisateur_id: '', module: '', categorie: '', q: '', requete_id: '', action: '', adresse_ip: '', attention: '' }; };
const queryString = auditQuery;
export function Audit({ bridge, state }: ScreenProps) {
  ...
  const load = useCallback(async () => {
    ...
    assertAuditCapabilities(result);
  ...
  useEffect(() => {
    if (!auto || busy || exporting || detail !== null || query.page !== 1) return;
    const timer = setInterval(() => { if (document.visibilityState === 'visible') setQuery(q => ({ ...q, bound: 0 })); }, 30000);
    return () => clearInterval(timer);
  }, [auto, busy, exporting, detail, query.page]);
  ...
  return <>
    <div className="audit-head"><div><ScreenTitle>{state.user?.super_administrateur ? 'Journal de sécurité' : 'Journal de traçabilité'}</ScreenTitle><p className="muted">{state.user?.super_administrateur ? 'Connexions, comptes, habilitations et incidents techniques — sans données métier' : company?.nom} · Heures de Lubumbashi · Données confidentielles</p></div><div className="audit-actions"><button className="btn" disabled={busy || exporting} onClick={() => { apply(query.filters); protection.refresh(); }}><i className="ti ti-refresh" aria-hidden="true" /> Actualiser</button><button className="btn" disabled={busy || exporting || !data?.total || !!error} onClick={() => download('xlsx')}><i className="ti ti-file-spreadsheet" aria-hidden="true" /> Excel</button><button className="btn" disabled={busy || exporting || !data?.total || !!error} onClick={() => download('pdf')}><i className="ti ti-file-type-pdf" aria-hidden="true" /> PDF</button></div></div>
    ...
    {state.user?.super_administrateur && <><div className="security-summary" aria-label="Indicateurs de la sélection">{[['Connexions réussies', data?.indicateurs.connexions], ['Échecs de connexion / mot de passe', data?.indicateurs.echecs], ['Accès refusés', data?.indicateurs.refus], ['Erreurs serveur', data?.indicateurs.erreurs], ['Traces d’habilitation', data?.indicateurs.habilitations]].map(([label, count]) => <div className="card" key={String(label)}><span>{label}</span><strong>{busy || error ? '—' : count ?? '—'}</strong></div>)}</div><p className="muted">Indicateurs calculés sur toute la sélection filtrée. Une même connexion à plusieurs sociétés compte une seule fois.</p><div className="audit-actions"><label><input type="checkbox" checked={auto} onChange={e => setAuto(e.target.checked)} /> Actualisation toutes les 30 secondes</label><button type="button" className="btn" onClick={() => { const f = { ...query.filters, attention: query.filters.attention ? '' : '1', action: '', categorie: '' }; setDraft(f); apply(f); }}>{query.filters.attention ? 'Voir tous les événements' : 'Afficher les événements à examiner'}</button></div>{auto && <p className="muted">L’actualisation se suspend pendant un export, l’examen d’une trace, sur les pages suivantes ou lorsque l’onglet est masqué.</p>}</>}
    <form className="card audit-filters" onSubmit={e => { e.preventDefault(); apply(draft); }}>
      <label>Périmètre<select className="form-input" value={draft.portee} onChange={e => { const filters = { ...draft, portee: e.target.value, utilisateur_id: '', module: '' }; setDraft(filters); apply(filters); }}>{state.user?.super_administrateur ? <option value="global">Utilisateurs et sécurité</option> : <><option value="societe">Société active</option><option value="partage">Clients / fournisseurs partagés</option>{state.companies.some(c => c.roles.includes('ADMIN_SYS')) && <option value="global">Sécurité et réglages globaux</option>}</>}</select></label>
      ...
      <label>Action<select className="form-input" value={draft.action} onChange={e => update('action', e.target.value)}><option value="">Toutes les actions</option>{data?.actions.map(a => <option key={a} value={a}>{auditActionLabel(a)}</option>)}</select></label>
      <label>Adresse IP exacte<input className="form-input" maxLength={45} value={draft.adresse_ip} onChange={e => update('adresse_ip', e.target.value)} placeholder="Ex. 192.0.2.10" /></label>
      <label>Action ou référence<input className="form-input" maxLength={100} value={draft.q} onChange={e => update('q', e.target.value)} placeholder="Action, table, référence ou IP…" /></label>
      ...
      <button className="btn btn-primary" disabled={exporting}>Appliquer les filtres</button><button className="btn" type="button" disabled={exporting} onClick={reset}><i className="ti ti-filter-off" aria-hidden="true" /> Réinitialiser</button>
    </form>
    <p className="muted audit-note">Une opération peut produire plusieurs traces liées par un identifiant de requête. Les champs RH sensibles et les secrets sont masqués. L’adresse IP est celle observée par Django ; derrière un proxy elle peut désigner le relais réseau. Les exports utilisent les filtres appliqués, même si vous préparez une autre recherche.</p>
...
function AuditDetail({ bridge, id, params, onClose, onRelated }: ScreenProps & { id: number; params: string; onClose: () => void; onRelated: (id: string) => void }) {
  ...
  return <Dialog bridge={bridge} title={`Trace n° ${id}${row ? ' · ' + auditActionLabel(row.action) : ''}`} onClose={onClose} footer={close => <><button className="btn" onClick={close}>Fermer</button>{row?.requete_id && <button className="btn btn-primary" onClick={() => { close(); onRelated(row.requete_id); }}>Voir les traces liées</button>}</>}>
