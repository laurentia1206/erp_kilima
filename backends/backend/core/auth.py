...
def empreinte_identifiants(password_hash):
    return hmac.new(settings.SECRET_KEY.encode(),password_hash.encode(),hashlib.sha256).hexdigest()


def creer_token(user_id: str, password_hash=None) -> str:
    payload = {"sub": str(user_id),
               "exp": datetime.now(timezone.utc) + timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES)}
    if password_hash is not None: payload['credentials']=empreinte_identifiants(password_hash)
...
class JWTAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        stamp=payload.get('credentials')
        if stamp is not None:
            if not isinstance(stamp,str) or not hmac.compare_digest(stamp,empreinte_identifiants(user.password_hash)):
                raise exceptions.AuthenticationFailed('Mot de passe modifié. Reconnectez-vous.')
        elif est_super_admin(user):
            raise exceptions.AuthenticationFailed('Reconnectez-vous pour accéder à la super administration.')
        return (user, None)
