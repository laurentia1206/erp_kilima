...
L'application utilise Django et Django REST Framework. L'interface servie par
Django se trouve encore dans `../backend/static/`. La base, les justificatifs
...
Ouvrir http://127.0.0.1:8000. Le lanceur historique
`../backend/Demarrer-ERP.bat` démarre également Django.
Le serveur de développement doit rester limité à l'usage local.
...
démarrage. Ils ne modifient pas `../backend/kilima_dev.db`. Node est requis
...
Les variables d'environnement du système priment sur `../backend/.env`.
