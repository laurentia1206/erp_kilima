...
from apps.rh.serializers import RHPaieMoisSerializer
from apps.rh.models import RHBulletin
from apps.rh.serializers import RHBulletinSerializer
from apps.rh.models import RHContrat
from apps.rh.serializers import RHContratSerializer
import hashlib
import json
...
class ContratViewSet(MetierModelViewSet):
    ...
    queryset = RHContrat.objects.none()
    serializer_class = RHContratSerializer
    lookup_url_kwarg = 'contrat_id'

    @action(detail=True, methods=['post'])
    @transaction.atomic
    def composition(self, request, contrat_id):
...
class PaieViewSet(MetierModelViewSet):
    """Ressource Paie ; contrats HTTP et validations métier conservés."""
    queryset = RHPaieMois.objects.none()
    serializer_class = RHPaieMoisSerializer
    lookup_url_kwarg = 'periode_id'

    def list(self, request):
        return self._traiter_periodes(request)
    ...
    @transaction.atomic
    def _traiter_periodes(self, request):
    ...
    def retrieve(self, request, periode_id):
    ...
    @action(detail=True, methods=['post'])
    @transaction.atomic
    def preparer(self, request, periode_id):
    ...
    @action(detail=True, methods=['post'])
    @transaction.atomic
    def decision(self, request, periode_id):
        sid,roles=contexte(request);o=objet(sid,periode_id);p=request.data;revision(o,p)
...
class BulletinViewSet(MetierModelViewSet):
    """Ressource Bulletin ; contrats HTTP et validations métier conservés."""
    queryset = RHBulletin.objects.none()
    serializer_class = RHBulletinSerializer
    lookup_url_kwarg = 'bulletin_id'

    @action(detail=True, methods=['post'])
    @transaction.atomic
    def payer(self, request, bulletin_id):
...
periodes = PaieViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='paie')
detail = PaieViewSet.as_view({'get': 'retrieve'}, http_method_names=['get', 'options'], detail=True, basename='paie')
composition = ContratViewSet.as_view({'post': 'composition'}, http_method_names=['post', 'options'], detail=True, basename='contrat')
preparer = PaieViewSet.as_view({'post': 'preparer'}, http_method_names=['post', 'options'], detail=True, basename='paie')
decision = PaieViewSet.as_view({'post': 'decision'}, http_method_names=['post', 'options'], detail=True, basename='paie')
payer = BulletinViewSet.as_view({'post': 'payer'}, http_method_names=['post', 'options'], detail=True, basename='bulletin')
