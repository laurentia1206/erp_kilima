...
from apps.rh.serializers import RHPolitiqueSerializer
from apps.rh.models import RHDette
from apps.rh.serializers import RHDetteSerializer
...
class PolitiqueViewSet(MetierModelViewSet):
    """Ressource Politique ; contrats HTTP et validations métier conservés."""
    queryset = RHPolitique.objects.none()
    serializer_class = RHPolitiqueSerializer
...
class AvanceJustifierViewSet(MetierViewSet):
    """Ressource AvanceJustifier ; contrats HTTP et validations métier conservés."""
...
class DetteAPayerViewSet(MetierViewSet):
    """Ressource DetteAPayer ; contrats HTTP et validations métier conservés."""
...
class DetteViewSet(MetierModelViewSet):
    """Ressource Dette ; contrats HTTP et validations métier conservés."""
    queryset = RHDette.objects.none()
    serializer_class = RHDetteSerializer
    lookup_url_kwarg = 'dette_id'

    def list(self, request):
        return self._traiter_dettes(request)
    ...
    @transaction.atomic
    def _traiter_dettes(self, request):
        sid,roles=contexte(request,LECTURE)
        if request.method=='POST':
    ...
    @transaction.atomic
    def decision(self, request, dette_id):
    ...
    def accord(self, request, dette_id):
        sid,roles=contexte(request,LECTURE)
    ...
    @action(detail=True, methods=['post'])
    @transaction.atomic
    def verser(self, request, dette_id):
...
parametres = PolitiqueViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='politique')
sources = AvanceJustifierViewSet.as_view({'get': 'sources'}, http_method_names=['get', 'options'], detail=False, basename='avance_justifier')
dettes = DetteViewSet.as_view({'get': 'list', 'post': 'create'}, http_method_names=['get', 'post', 'options'], detail=False, basename='dette')
decision = DetteViewSet.as_view({'post': 'decision'}, http_method_names=['post', 'options'], detail=True, basename='dette')
accord = DetteViewSet.as_view({'get': 'accord'}, http_method_names=['get', 'options'], detail=True, basename='dette')
a_payer = DetteAPayerViewSet.as_view({'get': 'a_payer'}, http_method_names=['get', 'options'], detail=False, basename='dette_a_payer')
verser = DetteViewSet.as_view({'post': 'verser'}, http_method_names=['post', 'options'], detail=True, basename='dette')
